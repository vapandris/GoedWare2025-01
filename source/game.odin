...
spirit_do_physical_AI :: proc(spirit: ^Spirit, dt: f32) {
    ...
    follow_radius_outer:f32 = 100 // spirit will not follow if player is further away
    ...
    if !spirit_is_far {
        ...
        if time.stopwatch_duration(spirit.attack_stopwatch) > SPIRIT_WARNING_DURATION {
            ...
            if !spirit.attacked_player && !spirit.got_attacked {
...
update :: proc() {
	...
	case .Paused: {
		...
		if rl.IsKeyPressed(.Q)      do run = false
	}}
}
